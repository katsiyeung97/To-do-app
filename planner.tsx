...
import {
  Plus, X, ChevronLeft, ChevronRight, Check, Cake, Droplet, Dumbbell, Pill,
  Trash2, Repeat, Flower2, Sun, CalendarDays, ListChecks, Sparkles, Moon, Leaf,
  Wallet, ShoppingCart, Receipt, Utensils, Car, ShoppingBag, Heart, Film,
  MoreHorizontal, TrendingUp, TrendingDown, Camera, Coffee, Wind, Feather, BookOpen,
  PiggyBank, Loader2, Landmark
} from "lucide-react";
...
function getOccurrences(task, rangeStartKey, rangeEndKey) {
  ...
  const step = (d) => {
    ...
    if (repeat === "monthly") return addMonths(d, 1);
    if (repeat === "yearly") return addYears(d, 1);
    return addDays(d, 1);
  };
  ...
  while (cur <= rangeEnd && guard < 3000) {
    if (cur >= rangeStart) result.push(toKey(cur));
    cur = step(cur);
    guard++;
  }
  return result;
...
function groupPeriodEpisodes(periodDays) {
  ...
  sorted.forEach((key) => {
    ...
    if (diffDays <= 1) current.push(key);
    else { episodes.push(current); current = [key]; }
  });
...
function computeCyclePhase(periodDays) {
  ...
  let phaseKey;
  if (isBleedingToday) phaseKey = "menstrual";
  else if (cycleDay <= 13) phaseKey = "follicular";
  else if (cycleDay <= 15) phaseKey = "ovulation";
...
function Bloom({ checked, onClick, size = 22 }) {
  return (
    ...
    >
      {checked && <Check className="check-pop" size={size * 0.62} color="#fff" strokeWidth={3} />}
    </button>
  );
}
...
function DragList({ items, getKey, onCommit, renderItem }) {
  ...
  const moveDrag = (e, id) => {
    if (!drag.current.active || dragId !== id) return;
    e.preventDefault();
  ...
  const endDrag = (id) => {
    if (drag.current.active) onCommit(order, id);
...
function DragHandleIcon() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 3, padding: "10px 8px", flexShrink: 0 }}>
      <div style={{ width: 16, height: 2, borderRadius: 1, background: COLORS.creamFaint }} />
      <div style={{ width: 16, height: 2, borderRadius: 1, background: COLORS.creamFaint }} />
    </div>
  );
}
  ...
  return (
    <div onClick={onCancel} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} className="modal-in" style={{
        ...
        <div style={{ fontFamily: FONT_DISPLAY, fontSize: 16, fontWeight: 600, color: COLORS.text, marginBottom: 4 }}>
          Delete this task?
        </div>
        <div style={{ fontSize: 12.5, color: COLORS.textMuted, marginBottom: 18 }}>This can't be undone.</div>
        <div style={{ display: "flex", gap: 8 }}>
          <button className="clean-btn" onClick={onCancel} style={{
          ...
          <button className="clean-btn" onClick={onConfirm} style={{
...
function SwipeRow({ onDelete, children }) {
  ...
  const onPointerUp = () => {
    if (drag.current.active) {
      if (dx < -REVEAL / 2) { setDx(-REVEAL); setRevealed(true); }
    ...
    <div style={{ position: "relative", overflow: "hidden" }}>
      <div style={{
        ...
        <button
          ...
          style={{ border: "none", background: "none", color: "#fff", fontSize: 12.5, fontWeight: 700, width: "100%", height: "100%" }}
        >
          Delete
        </button>
      </div>
      ...
      {confirming && (
        <ConfirmDeleteModal
          onCancel={() => { setConfirming(false); setDx(0); setRevealed(false); }}
          onConfirm={() => { setConfirming(false); setDx(0); setRevealed(false); onDelete(); }}
        />
      )}
    </div>
  );
...
function TaskRow({ task, dateKey, onToggle, onDelete, showDate, dragHandleProps }) {
  ...
  return (
    <div className="fade-up" style={{ display: "flex", alignItems: "stretch" }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <SwipeRow onDelete={() => onDelete(task.id)}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 4px" }}>
            {task.type === "birthday" ? (
              <div style={{
                ...
                <Cake size={13} color={COLORS.taupe} />
              </div>
            ) : (
              <Bloom checked={done} onClick={() => onToggle(task.id, dateKey)} />
            )}
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{
              ...
              {(time || (showDate && task.date) || isRepeating) && (
                <div style={{ display: "flex", gap: 8, marginTop: 2, alignItems: "center" }}>
                  {time && (
                    <span style={{ fontSize: 11.5, color: COLORS.cream, fontWeight: 700, textDecoration: "underline" }}>
                      {time}
                    </span>
                  )}
                  {showDate && task.date && (
                    <span style={{ fontSize: 11.5, color: COLORS.creamFaint }}>
                      {MONTHS[fromKey(dateKey).getMonth()]} {fromKey(dateKey).getDate()}
                    </span>
                  )}
                  {isRepeating && <Repeat size={11} color={COLORS.creamFaint} />}
                </div>
              )}
            </div>
          </div>
...
function MoodTile({ tile }) {
  ...
  return (
    <div className="fade-up" style={{
      ...
      <Icon size={26} color="rgba(255,255,255,0.85)" strokeWidth={1.5} style={{ position: "absolute", top: 14, left: 14 }} />
      <div style={{
        ...
        <div style={{ fontFamily: FONT_DISPLAY, fontStyle: "italic", fontSize: 14.5, color: "#fff", fontWeight: 500 }}>
          {tile.caption}
        </div>
      </div>
    </div>
...
function InspireView() {
  ...
  return (
    <div className="fade-up" style={{
      ...
      <Leaf size={90} color={COLORS.sage} style={{ position: "absolute", right: -20, top: -20, opacity: 0.12, transform: "rotate(20deg)" }} />
      <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: 1.6, textTransform: "uppercase", color: COLORS.sageDeep, marginBottom: 18, textAlign: "center" }}>
        Today's thought
      </div>
      <div style={{ fontFamily: FONT_DISPLAY, fontStyle: "italic", fontSize: 26, lineHeight: 1.5, color: COLORS.text, fontWeight: 500, textAlign: "center" }}>
        "{q.text}"
      </div>
      {q.author !== "unknown" && (
        <div style={{ fontSize: 13, color: COLORS.textMuted, marginTop: 16, textAlign: "center" }}>— {q.author}</div>
      )}
    </div>
  );
}
  ...
  return (
    <div className="fade-up">
      <Segmented
        value={which}
        onChange={setWhich}
        options={[{ label: "Today", value: "today" }, { label: "Tomorrow", value: "tomorrow" }]}
      />
      <div style={{ marginTop: 18 }}>
        <SectionLabel>

          {which === "today" ? "Today's schedule" : "Tomorrow's schedule"}
        </SectionLabel>
        {sorted.length === 0 ? (
          <EmptyState text="Nothing scheduled — enjoy the open space." />
        ) : (
          <DragList
            items={sorted}
            getKey={(t) => t.id}
            onCommit={onReorder}
            renderItem={(t, h) => <TaskRow task={t} dateKey={activeKey} onToggle={onToggle} onDelete={onDelete} dragHandleProps={h} />}
          />
        )}
      </div>
    </div>
  ...
  return (
    <div className="fade-up">
      ...
      {activeGeneral.length === 0 ? (
        <EmptyState text="No general tasks — add something to remember." />
      ) : (
        <DragList
          items={activeGeneral}
          getKey={(t) => t.id}
          onCommit={onReorder}
          renderItem={(t, h) => <TaskRow task={t} dateKey={t.date} onToggle={onToggle} onDelete={onDelete} dragHandleProps={h} />}
        />
      )}
      ...
      <div style={{ marginTop: 26 }}>
        ...
        >
          <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: COLORS.creamMuted }}>
            History ({history.length})
          </span>
          <ChevronRight size={16} color={COLORS.creamMuted} style={{
            transform: historyOpen ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.2s ease",
          }} />
        </button>
        {historyOpen && (
          history.length === 0 ? (
            <EmptyState text="Completed tasks will show up here." />
          ) : (
            history.map((h) => (
              <div key={h.id} className="fade-up" style={{
                ...
                <div style={{ flex: 1, fontSize: 13.5, color: COLORS.creamMuted, textDecoration: "line-through" }}>
                  {h.title}
                </div>
                {h.date && (
                  <div style={{ fontSize: 11, color: COLORS.creamFaint }}>
                    {MONTHS[fromKey(h.date).getMonth()]} {fromKey(h.date).getDate()}
                  </div>
                )}
              </div>
            ))
...
function MonthStructureModal({ initialDate, onClose }) {
  const months = [];
  for (let i = -12; i <= 12; i++) months.push(addMonths(initialDate, i));
  const todayKey = toKey(new Date());
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 16px 8px" }}>
          <div style={{ fontFamily: FONT_DISPLAY, fontSize: 15, fontWeight: 600, color: COLORS.bgDeep }}>Calendar</div>
          <button className="clean-btn" onClick={onClose} style={{ border: "none", background: COLORS.sageLight, borderRadius: "50%", padding: 6 }}>
            <X size={14} color={COLORS.bgDeep} />
          </button>
        </div>
        <div style={{ flex: 1, overflowY: "auto", scrollSnapType: "y mandatory" }}>
          {months.map((m, i) => (
            <div
              key={i}
              id={i === 12 ? "month-struct-current" : undefined}
              style={{
                scrollSnapAlign: "start", minHeight: "100%", padding: "8px 18px",
                display: "flex", flexDirection: "column", justifyContent: "center",
              }}
            >
              <div style={{ textAlign: "center", fontFamily: FONT_DISPLAY, fontSize: 17, fontWeight: 600, color: COLORS.bgDeep, marginBottom: 14 }}>
                {monthLabel(m)}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 5, marginBottom: 6 }}>
                {WEEKDAYS.map((w) => (
                  <div key={w} style={{ textAlign: "center", fontSize: 9.5, fontWeight: 700, color: COLORS.bgSoft }}>{w[0]}</div>
                ))}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 5 }}>
                {monthDayCells(m).map((c, idx) => (
                  <div key={idx} style={{ aspectRatio: "1 / 1", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    {c && (
                      <div style={{
                        ...
                        <span style={{ fontSize: 11, color: COLORS.bgDeep, fontWeight: c.key === todayKey ? 700 : 400 }}>{c.day}</span>
                      </div>
                    )}
                  </div>
                ))}
...
function CalendarView({ tasks, onToggle, onDelete, onReorder, refDate, setRefDate, zoom, setZoom }) {
  ...
  if (zoom === "day") { rangeStart = refDate; rangeEnd = refDate; }
  else if (zoom === "week") { rangeStart = refDate; rangeEnd = addDays(refDate, 6); }
  else if (zoom === "month") { rangeStart = startOfMonth(refDate); rangeEnd = endOfMonth(refDate); }
  else { rangeStart = startOfMonth(refDate); rangeEnd = endOfMonth(addMonths(refDate, 3)); }
  ...
  const step = (dir) => {
    if (zoom === "day") setRefDate(addDays(refDate, dir));
    else if (zoom === "week") setRefDate(addDays(refDate, dir * 7));
    else if (zoom === "month") setRefDate(addMonths(refDate, dir));
    else setRefDate(addMonths(refDate, dir * 4));
  };

  let rangeLabel;
  if (zoom === "day") rangeLabel = `${WEEKDAYS[refDate.getDay()]}, ${MONTHS[refDate.getMonth()]} ${refDate.getDate()}`;
  else if (zoom === "week") rangeLabel = `${MONTHS[rangeStart.getMonth()]} ${rangeStart.getDate()} – ${MONTHS[rangeEnd.getMonth()]} ${rangeEnd.getDate()}`;
  else if (zoom === "month") rangeLabel = `${MONTHS[refDate.getMonth()]} ${refDate.getFullYear()}`;
  else rangeLabel = `${MONTHS[rangeStart.getMonth()]} – ${MONTHS[rangeEnd.getMonth()]} ${rangeEnd.getFullYear()}`;
  ...
  return (
    <div className="fade-up">
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 10 }}>
        <button className="clean-btn" onClick={() => setStructureOpen(true)} style={{
          border: "none", background: COLORS.surfaceSoft2, borderRadius: 10, padding: "6px 10px",
          display: "flex", alignItems: "center", gap: 6,
        }}>
          <CalendarDays size={13} color={COLORS.bgDeep} />
          <span style={{ fontSize: 11, fontWeight: 600, color: COLORS.bgDeep }}>Month view</span>
        </button>
      </div>
      <Segmented
        value={zoom}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", margin: "16px 0 10px" }}>
        <button className="clean-btn" onClick={() => step(-1)} style={{ border: "none", background: COLORS.surfaceSoft2, borderRadius: 10, padding: 7 }}>
          <ChevronLeft size={16} color={COLORS.sageDeep} />
        </button>
        <div style={{ fontFamily: FONT_DISPLAY, fontSize: 15.5, fontWeight: 600, color: COLORS.cream }}>{rangeLabel}</div>
        <button className="clean-btn" onClick={() => step(1)} style={{ border: "none", background: COLORS.surfaceSoft2, borderRadius: 10, padding: 7 }}>
          <ChevronRight size={16} color={COLORS.sageDeep} />
        </button>
      </div>

      {dateKeys.length === 0 ? (
        <EmptyState text="Nothing on the calendar for this stretch." />
      ) : (
        dateKeys.map((k) => {
          const d = fromKey(k);
          ...
          return (
            <div key={k}>
              ...
              <div style={{ fontSize: 12, fontWeight: 700, color: COLORS.creamMuted, marginTop: 14, marginBottom: 2 }}>
                {formatDateHeader(k, todayKey, tomorrowKey)}
              </div>
              <DragList
                items={byDate[k]}
                getKey={(t) => t.id}
                onCommit={onReorder}
                renderItem={(t, h) => <TaskRow task={t} dateKey={k} onToggle={onToggle} onDelete={onDelete} dragHandleProps={h} />}
              />
            </div>
          );
        })
      ...
      {structureOpen && (
        <MonthStructureModal initialDate={refDate} onClose={() => setStructureOpen(false)} />
      )}
    </div>
  );
}
...
function HabitCard({ habit, onToggle, onDelete }) {
  ...
  if (!(habit.completions || []).includes(toKey(cursor))) cursor = addDays(cursor, -1);
    ...
    <div className="fade-up" style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
        <div style={{
          ...
          <Icon size={17} color={COLORS.sageDeep} />
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 14.5, fontWeight: 600, color: COLORS.text }}>{habit.name}</div>
          {streak > 0 && (
            <div style={{ fontSize: 11, color: COLORS.teal, fontWeight: 600 }}>{streak} day streak</div>
          )}
        </div>
        <button onClick={() => onDelete(habit.id)} style={{ border: "none", background: "none", padding: 6, color: COLORS.textFaint }}>
          <Trash2 size={13} />
        </button>
      </div>
        ...
        {week.map((d) => {
          ...
          return (
            <div key={k} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
              <span style={{ fontSize: 10, color: k === todayKey ? COLORS.sageDeep : COLORS.textFaint, fontWeight: k === todayKey ? 700 : 500 }}>
                {WEEKDAYS[d.getDay()][0]}
              </span>
              <button
                disabled={isFuture}
              ...
              >
                {done && <Check className="check-pop" size={14} color="#fff" strokeWidth={3} />}
              </button>
            </div>
          );
        })}
...
function ConsistencyCalendar({ habits }) {
  ...
  return (
    <div style={{ background: COLORS.surface, borderRadius: 20, padding: "18px 16px", marginTop: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
        <button className="clean-btn" onClick={() => setMonthRef(addMonths(monthRef, -1))} style={{ border: "none", background: COLORS.surfaceSoft2, borderRadius: 10, padding: 6 }}>
          <ChevronLeft size={14} color={COLORS.sageDeep} />
        </button>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: COLORS.textMuted }}>
          Consistency · {monthLabel(monthRef)}
        </div>
        <button className="clean-btn" onClick={() => setMonthRef(addMonths(monthRef, 1))} style={{ border: "none", background: COLORS.surfaceSoft2, borderRadius: 10, padding: 6 }}>
          <ChevronRight size={14} color={COLORS.sageDeep} />
        </button>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 6, marginBottom: 6 }}>
        {WEEKDAYS.map((w) => (
          <div key={w} style={{ textAlign: "center", fontSize: 9.5, color: COLORS.textFaint, fontWeight: 600 }}>{w[0]}</div>
        ))}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 6 }}>
        {cells.map((c, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "center", aspectRatio: "1 / 1" }}>
            {c && (
              <div style={{
      ...
      <div style={{ fontSize: 12, color: COLORS.textMuted, marginTop: 14, textAlign: "center" }}>
        {totalHabits === 0
...
function HabitsView({ habits, onToggle, onDelete }) {
  return (
    <div className="fade-up">
      ...
      {habits.length === 0 ? (
        <EmptyState text="No habits yet — add water, gym, vitamins, anything." />
      ) : (
        habits.map((h) => <HabitCard key={h.id} habit={h} onToggle={onToggle} onDelete={onDelete} />)
      )}
      <ConsistencyCalendar habits={habits} />
    </div>
  );
}
...
function weeklyBuckets(expenses, monthDate) {
  ...
  expenses.forEach((e) => {
    ...
    if (ed.getFullYear() !== monthDate.getFullYear() || ed.getMonth() !== monthDate.getMonth()) return;
  ...
  return (
    <div className="fade-up" style={{
      ...
      <div style={{
        ...
        <Icon size={15} color={item.type === "income" ? COLORS.sageDeep : COLORS.textMuted} />
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 14, fontWeight: 500, color: COLORS.text }}>{item.title}</div>
        <div style={{ fontSize: 11, color: COLORS.textFaint, marginTop: 1 }}>
          {item.type === "income" ? "Income" : item.category} · {MONTHS[fromKey(item.date).getMonth()]} {fromKey(item.date).getDate()}
        </div>
      </div>
      <div style={{ fontSize: 14, fontWeight: 700, color: item.type === "income" ? COLORS.sageDeep : COLORS.text, flexShrink: 0 }}>
        {item.type === "income" ? "+" : "−"}{formatMoney(item.amount)}
      </div>
      <button onClick={() => onDelete(item.id)} style={{ border: "none", background: "none", padding: 4, color: COLORS.textFaint, flexShrink: 0 }}>
        <Trash2 size={13} />
      </button>
    </div>
  );
...
function SpendingPulse({ pulse }) {
  ...
  if (weekPct === null && monthPct === null) {
    return (
      <div style={{ fontSize: 12.5, color: COLORS.creamMuted, fontStyle: "italic", fontFamily: FONT_DISPLAY, textAlign: "center", padding: "14px 8px" }}>
        Keep logging purchases — once there's a full week and month behind you, you'll see how your spending trends here.
      </div>
    );
  }

  const describe = (pct) => {
    if (pct === null) return "no prior data yet";
  ...
  return (
    <div style={{ textAlign: "center", padding: "14px 10px 4px" }}>
      <div style={{ fontSize: 13, color: COLORS.cream, lineHeight: 1.6 }}>
        This week you've spent <strong style={{ color: tone }}>{describe(weekPct)}</strong> last week.
        This month you're <strong style={{ color: tone }}>{describe(monthPct)}</strong> last month.
      </div>
    </div>
  );
}
  ...
  return (
    <div className="fade-up">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <button className="clean-btn" onClick={() => setMonthRef(addMonths(monthRef, -1))} style={{ border: "none", background: COLORS.surfaceSoft2, borderRadius: 10, padding: 7 }}>
          <ChevronLeft size={16} color={COLORS.sageDeep} />
        </button>
        <div style={{ fontFamily: FONT_DISPLAY, fontSize: 16, fontWeight: 600, color: COLORS.cream }}>{monthLabel(monthRef)}</div>
        <button className="clean-btn" onClick={() => setMonthRef(addMonths(monthRef, 1))} style={{ border: "none", background: COLORS.surfaceSoft2, borderRadius: 10, padding: 7 }}>
          <ChevronRight size={16} color={COLORS.sageDeep} />
        </button>
      </div>
      ...
      {/* Summary */}
      <div style={{ background: COLORS.surface, borderRadius: 20, padding: "18px 16px", display: "flex", marginBottom: 14 }}>
        <div style={{ flex: 1, textAlign: "center" }}>
          <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: 0.6, textTransform: "uppercase", color: COLORS.textFaint }}>Income</div>
          <div style={{ fontFamily: FONT_DISPLAY, fontSize: 18, fontWeight: 600, color: COLORS.sageDeep, marginTop: 4 }}>{formatMoney(income)}</div>
        </div>
        <div style={{ flex: 1, textAlign: "center", borderLeft: `1px solid ${COLORS.border}`, borderRight: `1px solid ${COLORS.border}` }}>
          <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: 0.6, textTransform: "uppercase", color: COLORS.textFaint }}>Spent</div>
          <div style={{ fontFamily: FONT_DISPLAY, fontSize: 18, fontWeight: 600, color: COLORS.danger, marginTop: 4 }}>{formatMoney(spent)}</div>
        </div>
        <div style={{ flex: 1, textAlign: "center" }}>
          <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: 0.6, textTransform: "uppercase", color: COLORS.textFaint }}>Net</div>
          <div style={{ fontFamily: FONT_DISPLAY, fontSize: 18, fontWeight: 600, color: COLORS.text, marginTop: 4 }}>{net >= 0 ? "+" : "−"}{formatMoney(net)}</div>
        </div>
      </div>

      {/* Categories & budgets */}
      <div style={{ background: COLORS.surface, borderRadius: 20, padding: "18px 16px", marginBottom: 14 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: COLORS.textMuted }}>Budgets</div>
          <button className="clean-btn" onClick={onOpenBudgets} style={{ border: "none", background: "none", display: "flex", alignItems: "center", gap: 4, padding: 2 }}>
            <PiggyBank size={13} color={COLORS.sageDeep} />
            <span style={{ fontSize: 11.5, color: COLORS.sageDeep, fontWeight: 600 }}>Edit</span>
          </button>
        </div>
        {activeCategories.length === 0 ? (
          <div style={{ fontSize: 12.5, color: COLORS.textFaint, fontStyle: "italic", fontFamily: FONT_DISPLAY }}>Nothing tracked yet this month.</div>
        ) : (
              ...
              <div key={c.key} style={{ marginBottom: 12 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 5 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                    ...
                    <span style={{ fontSize: 13, color: COLORS.text, fontWeight: 500 }}>{c.key}</span>
                  </div>
                  <span style={{ fontSize: 12, color: over ? COLORS.danger : COLORS.textMuted }}>
                    {formatMoney(spentC)}{budget > 0 ? ` / ${formatMoney(budget)}` : ""}
                  </span>
                </div>
                {budget > 0 && (
                  <div style={{ height: 6, borderRadius: 4, background: COLORS.surfaceSoft, overflow: "hidden" }}>
                    <div style={{
      ...
      <div style={{ background: COLORS.surface, borderRadius: 20, padding: "18px 16px", marginBottom: 14 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 12 }}>
          ...
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: COLORS.textMuted }}>Savings</div>
        </div>
        {savingsByAccount.length === 0 ? (
          <div style={{ fontSize: 12.5, color: COLORS.textFaint, fontStyle: "italic", fontFamily: FONT_DISPLAY }}>No savings tracked yet — log a deposit to RRSP, TFSA, or another account.</div>
        ) : (
          savingsByAccount.map((s) => (
            <div key={s.account} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 0" }}>
              <span style={{ fontSize: 13, color: COLORS.text, fontWeight: 500 }}>{s.account}</span>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 13.5, fontWeight: 700, color: COLORS.sageDeep }}>{formatMoney(s.month)} <span style={{ fontSize: 10.5, fontWeight: 500, color: COLORS.textFaint }}>this month</span></div>
                <div style={{ fontSize: 11, color: COLORS.textFaint }}>{formatMoney(s.allTime)} all-time</div>
              </div>
            </div>
          ))
        )}
      ...
      {/* Recent transactions */}
      <div style={{ background: COLORS.surface, borderRadius: 20, padding: "18px 16px", marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: COLORS.textMuted, marginBottom: 6 }}>
          Transactions
        </div>
        {sorted.length === 0 ? (
          <div style={{ fontSize: 12.5, color: COLORS.textFaint, fontStyle: "italic", fontFamily: FONT_DISPLAY, padding: "10px 0" }}>
            Nothing logged for {monthLabel(monthRef)} yet.
          </div>
        ) : (
          sorted.map((e) => <ExpenseRow key={e.id} item={e} onDelete={onDeleteExpense} />)
        )}
      </div>

      {/* Weekly bar chart — dark card, white bars */}
      <div style={{ background: COLORS.bgDeep, borderRadius: 20, padding: "18px 16px 14px" }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: COLORS.creamMuted, marginBottom: 14 }}>
          Weekly spending
        </div>
        <div style={{ display: "flex", alignItems: "flex-end", gap: 10, height: 110 }}>
          {buckets.map((b) => (
            <div key={b.label} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", height: "100%" }}>
              ...
              <div style={{ fontSize: 9.5, color: COLORS.creamFaint, marginTop: 6 }}>{b.label}</div>
            </div>
          ))}
        </div>
      </div>

      <SpendingPulse pulse={pulse} />
    </div>
  );
}
...
function QuickAddRow({ placeholder, onAdd }) {
  ...
  return (
    <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
      <input
        style={{ ...inputStyle, flex: 1, padding: "9px 12px", fontSize: 13.5 }}
        placeholder={placeholder}
        value={value}
        onChange={(e) => setValue(e.target.value)}
        onKeyDown={(e) => { if (e.key === "Enter") submit(); }}
      />
      <button className="clean-btn" onClick={submit} style={{
...
function GroceryView({ meals, setMeals, ingredients, pantry, onAddIngredient, onRemoveIngredient, onAddPantry, onRemovePantry, onBuyItem }) {
  ...
  return (
    <div className="fade-up">
      {/* Meal ideas */}
      <div style={{ background: COLORS.surface, borderRadius: 20, padding: "18px 16px", marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: COLORS.textMuted, marginBottom: 12 }}>
          This week's meals
        </div>
        {mealFields.map((m) => (
          <div key={m.key} style={{ marginBottom: 12 }}>
            ...
            <input
              ...
              placeholder={`What's for ${m.label.toLowerCase()} this week?`}
              value={meals[m.key] || ""}
      ...
      {/* Ingredients needed */}
      <div style={{ background: COLORS.surface, borderRadius: 20, padding: "18px 16px", marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: COLORS.textMuted, marginBottom: 4 }}>
          Ingredients needed
        </div>
        {ingredients.length === 0 ? (
          <div style={{ fontSize: 12.5, color: COLORS.textFaint, fontStyle: "italic", fontFamily: FONT_DISPLAY, padding: "8px 0" }}>
            Add what your meals need — we'll check it against what's in stock.
          </div>
        ) : (
          ingredients.map((i) => {
            const have = pantryNames.has(i.name.trim().toLowerCase());
            return (
              <div key={i.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 0" }}>
                <span style={{ flex: 1, fontSize: 13.5, color: COLORS.text }}>{i.name}</span>
                {have && (
                  <span style={{
                ...
                <button onClick={() => onRemoveIngredient(i.id)} style={{ border: "none", background: "none", padding: 4, color: COLORS.textFaint }}>
                  <Trash2 size={13} />
                </button>
              </div>
            );
        ...
        <QuickAddRow placeholder="Add an ingredient…" onAdd={onAddIngredient} />
      </div>

      {/* To buy — filtered result */}
      <div style={{ background: COLORS.surface, borderRadius: 20, padding: "18px 16px", marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: COLORS.textMuted, marginBottom: 8 }}>
          To buy ({toBuy.length})
        </div>
        {toBuy.length === 0 ? (
          <div style={{ fontSize: 12.5, color: COLORS.textFaint, fontStyle: "italic", fontFamily: FONT_DISPLAY, padding: "8px 0" }}>
            You have everything you need. Nothing to buy.
          </div>
        ) : (
          toBuy.map((i) => (
            <div key={i.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0" }}>
              <Bloom checked={false} onClick={() => onBuyItem(i)} />
              <span style={{ fontSize: 14, color: COLORS.text, fontWeight: 500 }}>{i.name}</span>
            </div>
          ))
        )}
      </div>
      ...
      {/* In stock — pantry */}
      <div style={{ background: COLORS.surface, borderRadius: 20, padding: "18px 16px" }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: COLORS.textMuted, marginBottom: 4 }}>
          In stock
        </div>
        {pantry.length === 0 ? (
          <div style={{ fontSize: 12.5, color: COLORS.textFaint, fontStyle: "italic", fontFamily: FONT_DISPLAY, padding: "8px 0" }}>
            Nothing marked as in stock yet.
          </div>
        ) : (
          pantry.map((p) => (
            <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 0" }}>
              <span style={{ flex: 1, fontSize: 13.5, color: COLORS.text }}>{p.name}</span>
              <button onClick={() => onRemovePantry(p.id)} style={{ border: "none", background: "none", padding: 4, color: COLORS.textFaint }}>
                <Trash2 size={13} />
              </button>
            </div>
          ))
        )}
        <QuickAddRow placeholder="Add to in-stock…" onAdd={onAddPantry} />
      </div>
    </div>
  );
}
...
function PhaseStepper({ currentKey }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 3, marginBottom: 16, flexWrap: "wrap" }}>
      {PHASE_ORDER.map((k, i) => (
        <React.Fragment key={k}>
          ...
          {i < PHASE_ORDER.length - 1 && <ChevronRight size={10} color={COLORS.textFaint} />}
        </React.Fragment>
      ))}
    </div>
  );
...
function CycleChart({ cycle }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="130" style={{ overflow: "visible" }}>
      {bands.map((b, i) => (
        <rect key={i} x={dayX(b.from)} y={0} width={dayX(b.to) - dayX(b.from)} height={chartH} fill={b.color} opacity={0.08} />
      ))}
      <polyline points={toPoints(HORMONE_CURVES.Estrogen)} fill="none" stroke={HORMONE_COLORS.Estrogen} strokeWidth="2" strokeLinejoin="round" />
      <polyline points={toPoints(HORMONE_CURVES.Progesterone)} fill="none" stroke={HORMONE_COLORS.Progesterone} strokeWidth="2" strokeLinejoin="round" />
      <polyline points={toPoints(HORMONE_CURVES["LH surge"])} fill="none" stroke={HORMONE_COLORS["LH surge"]} strokeWidth="1.6" strokeLinejoin="round" />
      <polyline points={toPoints(HORMONE_CURVES.FSH)} fill="none" stroke={HORMONE_COLORS.FSH} strokeWidth="1.6" strokeDasharray="3,2" strokeLinejoin="round" />
      {markerX !== null && (
        <>
          <line x1={markerX} y1={0} x2={markerX} y2={chartH} stroke={COLORS.text} strokeWidth="1" strokeDasharray="2,2" />
          <text x={markerX} y={H - 2} fontSize="7" fill={COLORS.textMuted} textAnchor="middle">today</text>
        </>
      )}
      <text x={dayX(1)} y={H - 2} fontSize="7" fill={COLORS.textFaint} textAnchor="start">day 1</text>
      <text x={dayX(28)} y={H - 2} fontSize="7" fill={COLORS.textFaint} textAnchor="end">day 28</text>
    </svg>
  );
}
...
function PeriodView({ periodDays, onToggleDay }) {
  ...
  return (
    <div className="fade-up">
      {/* Calendar for marking period days */}
      <div style={{ background: COLORS.surface, borderRadius: 20, padding: "18px 16px", marginBottom: 14 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          <button className="clean-btn" onClick={() => setMonthRef(addMonths(monthRef, -1))} style={{ border: "none", background: COLORS.surfaceSoft2, borderRadius: 10, padding: 6 }}>
            <ChevronLeft size={14} color={COLORS.sageDeep} />
          </button>
          <div style={{ fontSize: 13, fontFamily: FONT_DISPLAY, fontWeight: 600, color: COLORS.text }}>{monthLabel(monthRef)}</div>
          <button className="clean-btn" onClick={() => setMonthRef(addMonths(monthRef, 1))} style={{ border: "none", background: COLORS.surfaceSoft2, borderRadius: 10, padding: 6 }}>
            <ChevronRight size={14} color={COLORS.sageDeep} />
          </button>
        </div>
          ...
          {cells.map((c, i) => (
            <div key={i} style={{ aspectRatio: "1 / 1", display: "flex", alignItems: "center", justifyContent: "center" }}>
              {c && (
                ...
                >
                  <span style={{ fontSize: 11, color: periodSet.has(c.key) ? "#fff" : COLORS.textMuted, fontWeight: c.key === todayKey ? 700 : 400 }}>
                    {c.day}
                  </span>
                </button>
              )}
        ...
        <div style={{ fontSize: 11, color: COLORS.textFaint, marginTop: 10, textAlign: "center" }}>Tap a day to mark it as a period day</div>
      </div>

      {/* Phase summary */}
      <div style={{ background: COLORS.surface, borderRadius: 20, padding: "18px 16px" }}>
        {!phase ? (
          <div style={{ fontSize: 12.5, color: COLORS.textFaint, fontStyle: "italic", fontFamily: FONT_DISPLAY, textAlign: "center", padding: "10px 0" }}>
            Mark a few period days above and your current phase will show up here.
          </div>
        ) : (
          <>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: COLORS.textMuted, marginBottom: 4 }}>
            ...
            <div style={{ fontFamily: FONT_DISPLAY, fontSize: 20, fontWeight: 600, color: COLORS.sageDeep, marginBottom: 2 }}>{phase.label}</div>
            <div style={{ fontSize: 12, color: COLORS.textFaint, marginBottom: 14 }}>{phase.range} · cycle day {cycle.cycleDay}</div>

            {cycle.longCycle && (
              <div style={{ fontSize: 12, color: COLORS.danger, marginBottom: 14 }}>
                This cycle is running longer than the typical ~28–35 days. If that keeps happening, it's worth mentioning to a doctor.
              </div>
            )}

            <PhaseStepper currentKey={cycle.phaseKey} />

            <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: 0.8, textTransform: "uppercase", color: COLORS.textMuted, marginBottom: 4 }}>
              Cycle overview
            </div>
            <CycleChart cycle={cycle} />

            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 10, marginBottom: 14 }}>
              {["Estrogen", "Progesterone", "LH surge", "FSH"].map((t) => (
                <TermChip key={t} word={t} dotColor={HORMONE_COLORS[t]} active={activeTerm === t} onClick={(w) => setActiveTerm((cur) => (cur === w ? null : w))} />
              ))}
            </div>

            {activeTerm && (
              <div style={{ background: COLORS.sageLight, borderRadius: 14, padding: "12px 14px", marginBottom: 16, position: "relative" }}>
                <button className="clean-btn" onClick={() => setActiveTerm(null)} style={{ position: "absolute", top: 8, right: 8, border: "none", background: "none", padding: 4 }}>
                  <X size={12} color={COLORS.sageDeep} />
                </button>
                <div style={{ fontWeight: 700, fontSize: 12.5, color: COLORS.sageDeep, marginBottom: 4, paddingRight: 18 }}>{activeTerm}</div>
                <div style={{ fontSize: 12.5, color: COLORS.text, lineHeight: 1.5 }}>{GLOSSARY[activeTerm]}</div>
              </div>
            )}

            <div style={{ fontSize: 13, color: COLORS.text, lineHeight: 1.55, marginBottom: 18 }}>{phase.hormoneNote}</div>

            {Object.entries(phase.tips).map(([category, items]) => (
              <div key={category} style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 11.5, fontWeight: 700, color: COLORS.textMuted, marginBottom: 6 }}>{category}</div>
                {items.map((tip, i) => (
                  <div key={i} style={{ fontSize: 13, color: COLORS.text, lineHeight: 1.5, marginBottom: 3, paddingLeft: 12, position: "relative" }}>
            ...
            <div style={{ fontSize: 11, color: COLORS.textFaint, marginTop: 4, fontStyle: "italic" }}>
              General wellness info, not medical advice — check with a healthcare provider for anything persistent or concerning.
            </div>
          </>
        )}
      </div>
    ...
    <div style={{
      ...
      >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
          <div style={{ fontFamily: FONT_DISPLAY, fontSize: 19, fontWeight: 600, color: COLORS.text }}>{title}</div>
          <button className="clean-btn" onClick={onClose} style={{ border: "none", background: COLORS.surfaceSoft2, borderRadius: "50%", padding: 7 }}>
...
function FieldLabel({ children }) {
  return <div style={{ fontSize: 11.5, fontWeight: 700, color: COLORS.textMuted, marginBottom: 6, letterSpacing: 0.3 }}>{children}</div>;
}
...
function AddTaskModal({ onClose, onSave, defaultDate }) {
  ...
  return (
    <ModalShell title={isBirthday ? "Add a birthday" : "Add a task"} onClose={onClose}>
      <FieldLabel>Type</FieldLabel>
      <div style={{ marginBottom: 16 }}>
        <Segmented value={type} onChange={setType} options={[{ label: "Task", value: "task" }, { label: "Birthday", value: "birthday" }]} />
      </div>

      <FieldLabel>{isBirthday ? "Whose birthday?" : "Title"}</FieldLabel>
      <input
        style={{ ...inputStyle, marginBottom: 16, fontFamily: FONT_BODY }}
        placeholder={isBirthday ? "e.g. Hannah" : "e.g. Check in with Hannah"}
        value={title}
      ...
      {!isBirthday && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
          ...
          <span style={{ fontSize: 13.5, color: COLORS.text }}>Add a date</span>
        </div>
      )}
      ...
      {effectiveHasDate && (
        <>
          ...
          <input type="date" style={{ ...inputStyle, marginBottom: 16 }} value={date} onChange={(e) => setDate(e.target.value)} />
          ...
          {!isBirthday && (
            <>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
                ...
                <span style={{ fontSize: 13.5, color: COLORS.text }}>Add a time</span>
              </div>
              {hasTime && (
                <>
                  <FieldLabel>Time</FieldLabel>
                  <input type="time" style={{ ...inputStyle, marginBottom: 16 }} value={time} onChange={(e) => setTime(e.target.value)} />
                </>
              )}

              <FieldLabel>Repeat</FieldLabel>
              <select style={{ ...inputStyle, marginBottom: 16 }} value={repeat} onChange={(e) => setRepeat(e.target.value)}>
...
function AddHabitModal({ onClose, onSave }) {
  ...
  return (
    <ModalShell title="Add a habit" onClose={onClose}>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        {Object.entries(HABIT_ICONS).map(([key, Icon]) => (
          ...
          >
            <Icon size={18} color={icon === key ? COLORS.sageDeep : COLORS.textMuted} />
          </button>
        ))}
      </div>
      <button
        className="clean-btn"
        onClick={() => { if (canSave) { onSave({ id: uid(), name: name.trim(), icon, completions: [] }); onClose(); } }}
        disabled={!canSave}
        style={{
...
function AddExpenseModal({ onClose, onSaveExpense, onSaveSavings }) {
  ...
  return (
    <ModalShell title={titleText} onClose={onClose}>
      <div style={{ marginBottom: 16 }}>
        <Segmented
          value={mode}
          onChange={setMode}
      ...
      {mode === "expense" && (
        <label className="clean-btn" style={{
          ...
          {scanning ? <Loader2 size={16} className="check-pop" /> : <Camera size={16} />}
          <span>{scanning ? "Reading receipt…" : "Scan a receipt"}</span>
          <input type="file" accept="image/*" capture="environment" onChange={handleScan} style={{ display: "none" }} disabled={scanning} />
        </label>
      )}
      {scanError && (
        <div style={{ fontSize: 12, color: COLORS.danger, marginTop: -10, marginBottom: 14 }}>{scanError}</div>
      )}

      {mode === "savings" ? (
        <>
          <FieldLabel>Account</FieldLabel>
          <select style={{ ...inputStyle, marginBottom: 16 }} value={account} onChange={(e) => setAccount(e.target.value)}>
            {SAVINGS_ACCOUNTS.map((a) => <option key={a} value={a}>{a}</option>)}
          </select>
        </>
      ) : (
        <>
          <FieldLabel>{mode === "expense" ? "Merchant" : "Source"}</FieldLabel>
          <input
            style={{ ...inputStyle, marginBottom: 16 }}
      ...
      {mode === "expense" && (
        <>
          ...
          <select style={{ ...inputStyle, marginBottom: 16 }} value={category} onChange={(e) => setCategory(e.target.value)}>
...
function BudgetModal({ budgets, onClose, onSave }) {
  ...
  return (
    <ModalShell title="Monthly budgets" onClose={onClose}>
      {CATEGORIES.map((c) => (
        <div key={c.key} style={{ marginBottom: 14 }}>
          <FieldLabel>{c.key}</FieldLabel>
          <input
            type="number" inputMode="decimal" step="1"
            style={inputStyle}
            placeholder="0"
            value={draft[c.key] ?? ""}
            onChange={(e) => setDraft((d) => ({ ...d, [c.key]: parseFloat(e.target.value) || 0 }))}
          />
        </div>
      ))}
      <button
...
export default function App() {
  ...
  const reorderTasks = (newOrderIds, draggedId) => {
    setTasks((prev) => {
      ...
      return prev.map((t) => {
        ...
        if (t.id === draggedId) updated.time = null;
        return updated;
      });
    });
  };
  ...
  return (
    <div className="planner-root" style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            <div style={{
              ...
              <span className="intro-do">do</span>
            </div>
            <div style={{
              ...
              {introChecked && <Check className="check-pop" size={15} color="#fff" strokeWidth={3} />}
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ width: "100%", maxWidth: 440, minHeight: "100vh", display: "flex", flexDirection: "column", position: "relative", zIndex: 1 }}>
        {/* Header */}
        <div style={{ padding: "calc(env(safe-area-inset-top, 20px) + 18px) 20px 8px" }}>
          <div style={{ fontFamily: FONT_LOGO, fontSize: 24, fontWeight: 500, letterSpacing: 0.8, ...emboss }}>
            to/do
          </div>
          <div style={{ fontSize: 12.5, color: COLORS.creamMuted, marginTop: 6 }}>{dateStr}</div>
        </div>

        {/* Content */}
        <div style={{ flex: 1, padding: "16px 20px 110px", overflowY: "auto", position: "relative" }}>
          <div key={tab} className="tab-panel">
            {tab === "today" && <TodayView tasks={tasks} onToggle={toggleTask} onDelete={deleteTask} onReorder={reorderTasks} />}
            {tab === "tasks" && <TasksView tasks={tasks} onToggle={toggleTask} onDelete={deleteTask} onReorder={reorderTasks} />}
            {tab === "calendar" && (
              <CalendarView
                tasks={tasks} onToggle={toggleTask} onDelete={deleteTask} onReorder={reorderTasks}
                refDate={calRefDate} setRefDate={setCalRefDate} zoom={calZoom} setZoom={setCalZoom}
              />
            )}
            {tab === "habits" && <HabitsView habits={habits} onToggle={toggleHabit} onDelete={deleteHabit} />}
            {tab === "expenses" && (
              <ExpensesView
                expenses={expenses} budgets={budgets} savings={savings}
                monthRef={expenseMonthRef} setMonthRef={setExpenseMonthRef}
                onDeleteExpense={deleteExpense} onOpenBudgets={() => setBudgetModalOpen(true)}
              />
            )}
            {tab === "grocery" && (
              <GroceryView
                meals={meals} setMeals={setMeals} ingredients={ingredients} pantry={pantry}
                onAddIngredient={addIngredient} onRemoveIngredient={removeIngredient}
                onAddPantry={addPantryItem} onRemovePantry={removePantryItem} onBuyItem={buyIngredient}
              />
            )}
            {tab === "period" && <PeriodView periodDays={periodDays} onToggleDay={togglePeriodDay} />}
            {tab === "quote" && <InspireView />}
          </div>
        </div>

        {/* Floating add button — sits beside the list, thumb-reachable */}
        {tab !== "quote" && tab !== "grocery" && tab !== "period" && (
        <button
          className="clean-btn"
        ...
        >
          <Plus size={16} color={COLORS.sageDeep} strokeWidth={2.4} />
        </button>
        )}
        ...
        <div style={{
          ...
          {TABS.map((t) => {
            ...
            return (
              ...
              >
                <Icon size={18} color={active ? COLORS.sageDeep : COLORS.textFaint} strokeWidth={active ? 2.3 : 1.8} />
                <span style={{ fontSize: 9.5, fontWeight: active ? 700 : 500, color: active ? COLORS.sageDeep : COLORS.textFaint, whiteSpace: "nowrap" }}>
                  {t.label}
                </span>
              </button>
            );
      ...
      {modalOpen && tab === "habits" && (
        <AddHabitModal onClose={() => setModalOpen(false)} onSave={addHabit} />
      )}
      {modalOpen && tab === "expenses" && (
        <AddExpenseModal onClose={() => setModalOpen(false)} onSaveExpense={addExpense} onSaveSavings={addSavings} />
      )}
      {modalOpen && tab !== "habits" && tab !== "expenses" && tab !== "grocery" && tab !== "quote" && tab !== "period" && (
        <AddTaskModal onClose={() => setModalOpen(false)} onSave={addTask} defaultDate={defaultTaskDate} />
      )}
      {budgetModalOpen && (
        <BudgetModal budgets={budgets} onClose={() => setBudgetModalOpen(false)} onSave={saveBudgets} />
      )}
    </div>
  );
}
